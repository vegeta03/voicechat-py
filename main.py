...
def transcribe_with_groq(audio_file_path, api_key=None):
    ...
    with open(audio_file_path, "rb") as file:
        transcription = client.audio.transcriptions.create(
            file=file,
            model="whisper-large-v3-turbo",  # Using the turbo version for faster transcription
            response_format="json",  # Changed from "text" to "json"
            language="en"  # English language
        )
    
    return transcription.text  # Now properly accessing the text attribute of the JSON response
...
def main():
    ...
    try:
        ...
        if success:
            # Ask for user confirmation before sending to Groq
            confirm = input(f"Do you want to send the recorded audio file '{output_filename}' to Groq for transcription? (y/n): ").strip()
            if confirm.lower() == 'y':
    ...
    # Note: We're no longer deleting the audio file so it remains in the current directory
